...
public class NotificationCenter : MonoBehaviour
{
    ...
    public void AddObserver ( NotificationReceiver observer , Notification.notifications name )
    {
        ...
        {
            notifications[ name ] = new List<NotificationReceiver> ();
        }

        List<NotificationReceiver> notifyList = notifications[ name ] as List<NotificationReceiver>;
    ...
    public void RemoveObserver ( NotificationReceiver observer , Notification.notifications name )
    {
        List<NotificationReceiver> notifyList = ( List<NotificationReceiver> ) notifications[ name ];
    ...
    public void PostNotification ( Object sender , Notification.notifications aName , NotificationReceiver target = null )
    {
        PostNotification ( sender , aName , null , target );
    }

    public void PostNotification ( Object sender , Notification.notifications aName , Hashtable aData , NotificationReceiver target = null )
    {
        PostNotification ( new Notification ( sender , aName , aData ) , target );
    }

    public void PostNotification ( Notification aNotification , NotificationReceiver target = null )
    {
        // First make sure that the name of the notification is valid.
        //Debug.Log("sender: " + aNotification.name);
        /* if ( aNotification.name == null )
        {
            Debug.Log ( "Null name sent to PostNotification." );
            return;
        }*/
        // Obtain the notification list, and make sure that it is valid as well
        List<NotificationReceiver> notifyList = ( List<NotificationReceiver> ) notifications[ aNotification.name ];
        if ( notifyList == null )
        {
        ...
        foreach ( NotificationReceiver observer in notifyList )
        {
            ...
            else
            {
                ...
                if ( target == null )
                    ...
                    observer.receiveNotification ( aNotification );
            }
        }

        // Remove all the invalid observers
        foreach ( NotificationReceiver observer in observersToRemove )
        {
